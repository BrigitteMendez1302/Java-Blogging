...
public class TagServiceImpl implements TagService{
    ...
    public Page<Tag> getAllTagsByPostId(Long postId, Pageable pageable) {
        return postRepository.findById(postId).map( post ->{
            //I need the collection, the quantity of the collection
            //and a pageable for pages
    ...
    public ResponseEntity<?> deleteTag(Long tagId) {
        return tagRepository.findById(tagId).map(tag ->
        {
            ...
            return ResponseEntity.ok().build(); //body builder es una clase que genera una instancia de este nombre
        }).orElseThrow(()-> new ResourceNotFoundException("Tag", "Id", tagId));
    }
}